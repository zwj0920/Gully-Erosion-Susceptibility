...
data = pd.read_excel(file_path, sheet_name=3)  # 使用 sheet_name=3，即沟深的数据
...
# 定义模型公式  
formula = '沟深 ~ ndvi + spi + twi + cm + aspect + slope + dem + artificial'  
...
# 计算MAE和R²  
train_mae = mean_absolute_error(normalized_data['沟深'], normalized_data['Predicted'])  
train_r2 = r2_score(normalized_data['沟深'], normalized_data['Predicted'])  
...
sns.scatterplot(x='沟深', y='Predicted', data=normalized_data, color='blue', alpha=0.6)  
plt.plot([normalized_data['沟深'].min(), normalized_data['沟深'].max()],   
         [normalized_data['沟深'].min(), normalized_data['沟深'].max()],   
         'r--', linewidth=2, label='y=x')  
plt.xlabel('Actual', fontsize=18)  
plt.ylabel('Predicted', fontsize=18)  
plt.title('Comparison of Actual and Predicted Gully Depth', fontsize=20)  
...
plt.title('Residual Distribution of Gully Depth', fontsize=20)
plt.grid(True)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.tight_layout()
...
plt.title('Q-Q Plot of Gully Depth Residuals', fontsize=20)
