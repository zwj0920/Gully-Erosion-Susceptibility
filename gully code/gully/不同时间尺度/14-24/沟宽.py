...
sns.scatterplot(x='沟宽', y='Predicted', data=normalized_data, color='blue', alpha=0.6)  
plt.plot([normalized_data['沟宽'].min(), normalized_data['沟宽'].max()],   
         [normalized_data['沟宽'].min(), normalized_data['沟宽'].max()],   
         'r--', linewidth=2, label='y=x')  
plt.xlabel('Actual', fontsize=18)  
plt.ylabel('Predicted', fontsize=18)  
plt.title('Comparison of Actual and Predicted Gully Width', fontsize=20)  
...
plt.title('Residual Distribution of Gully Width', fontsize=20)
plt.grid(True)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.tight_layout()
...
plt.title('Q-Q Plot of Gully Width Residuals', fontsize=20)
